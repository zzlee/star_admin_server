                    ...
                    UGCs.find({"no": data.content.no}).exec(function(err_2, result_2){
                        if(!err_2) {
                            if(typeof result_2[0].userRawContent[0] === 'undefined'){
                                LiveContentListInfo(data.content.no, result, data.timeslot.start, data.timeslot.end, data.liveState, data.playState, data.content.ownerId.fbUserId, data._id, data.content.ownerId._id, data.canBeFoundInPlayerLog, result_2[0].url.s3, null, result_2[0].contentClass, liveContentList);
